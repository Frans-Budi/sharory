...
import com.fransbudikashira.storyapp.databinding.ActivityRegisterBinding
...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun playAnimation() {
        ...
        val button = ObjectAnimator.ofFloat(binding.registerButton, View.ALPHA, 1f).setDuration(300)

        AnimatorSet().apply {
            playSequentially(title, edtName, edtEmail, edtPassword, button)
            start()
        }
    }

    private fun setupEditText(viewModel: RegisterViewModel) {
        // Name
        binding.edtName.addTextChangedListener(object: TextWatcher{
            override fun beforeTextChanged(s: CharSequence?, start: Int, count: Int, after: Int) {}
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                if (!binding.edtName.error.isNullOrEmpty()) {
                    checkName = false
    ...
    private fun isEnabledButton(viewModel: RegisterViewModel) {
